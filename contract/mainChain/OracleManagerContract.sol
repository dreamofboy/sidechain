...
contract OracleManagerContract is Ownable {
    ...
    function checkOracles(bytes32 dataHash, uint256 nonce, bytes[] memory sigList, address[] memory signOracles) internal returns (bool) {
        ...
        if (false && multivalidatesignSwitch) {
            uint256 signFlag = 0;
    ...
    function checkOraclesWithMultiValidate(bytes32 dataHash, uint256 nonce, bytes[] memory _sigList, address[] memory _oracleList) internal returns (bool) {
/*
        SignMsg storage signMsg = withdrawMultiSignList[nonce][dataHash];

        bytes32 ret = batchvalidatesign(dataHash, 
        _sigList, 
        _oracleList);
